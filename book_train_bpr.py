...
from book_gcn_attention import *
import numpy as np
import random
...
class BPR1:
    path_to_trained_pmf = 'data/trained_TransE_book.pt'
    embedding_funcation = TransE(77903, 25, device)
    embedding_funcation.load_state_dict(torch.load(path_to_trained_pmf))
    entity_embedding = embedding_funcation.entities_emb.weight.data
    embeddings = entity_embedding[:77903, :]
    item_count = embeddings.shape[0]  # self.item_count = 77903
    emb_size = embeddings.shape[1]
    output = GraphEncoder(item_count, emb_size, user_num=17860, embeddings=embeddings, max_seq_length=10, max_node=20,
                          hiddim=20,
    ...
    def train2(self, user_items_dict):
        lr = 0.01  # 步长α
        reg = 0.01  # 参数λ
        train_count = 1700  #训练次数
        beginUserId = 77903
        # 设定的用户矩阵
        U = np.zeros((len(user_items_dict), 20))
    ...
    def main(self):
        user_items_dict = self.load_data('data/book_rating_kg1')
        # # 测试train2
        ...
        #     scores = Counter({idx: val for idx, val in enumerate(user_scoreList_dict[userr]) if idx not in user_items_dict[userr+77903]})
        #     topk = scores.most_common(5)
        #     print(f'{userr}_top5:{topk}')

        # 测试train3
        # new_scores = self.train3(77903,user_items_dict,5)
        # print(new_scores)

        # 测试train4
        renew_scores = self.train4(77903, user_items_dict, [5])
        print(renew_scores)
